class Trip < ApplicationRecord
  validates :source, format: { with: /\A[a-zA-Z]+\z/, message: "only allows letters" }, length: { maximum: 255, too_long: "%{count} characters is the maximum allowed" }
  validates :destination, format: { with: /\A[a-zA-Z]+\z/, message: "only allows letters" }, length: { maximum: 255, too_long: "%{count} characters is the maximum allowed" }
